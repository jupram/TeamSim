...
- Every person has a configurable skill-score distribution with editable `mean`, `variance`, and distribution family.
...
- Configure team names, person names, distribution family, score means, and variances inline.
- Survival panel showing how many simulation steps each member and team survived.
...
Each person can use one of these distribution families:
...
The simulator keeps `mean` and `variance` as the shared controls. Different families interpret those controls differently, which changes how often extreme sampled scores occur.
